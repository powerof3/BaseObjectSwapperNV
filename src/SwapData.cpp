...
namespace FormSwap
{
	...
	bool ObjectData::HasValidProperties(const TESObjectREFR* a_ref) const
	{
	...
	void ObjectData::GetProperties(const std::string& a_path, const std::string& a_str, std::function<void(RE::FormID, ObjectData&)> a_func)
	{
		...
		if (const auto baseFormID = util::GetFormID(formPair[0]); baseFormID != 0) {
			...
			_ERROR("\t\t\t\tfail : [%s] (BASE formID not found)", a_str);
		}
	}
	...
	TESBoundObject* SwapFormData::GetSwapBase(const TESObjectREFR* a_ref) const
	{
		...
		if (const auto baseFormID = util::GetFormID(formPair[0]); baseFormID != 0) {
			if (const auto swapFormID = util::GetSwapFormID(formPair[1]); !swap_empty(swapFormID)) {
				...
				if (base_same_as_swap(baseFormID, swapFormID) && !distribution::is_valid_entry(properties)) {
					_ERROR("\t\t\t\tfail : [%s] (BASE formID == SWAP formID)", a_str);
					return;
				}
				...
				_ERROR("\t\t\t\tfail : [%s] (SWAP formID not found)", a_str);
			}
			...
			if (auto swapFormIDs = util::GetFormIDOrderedSet(formPair[1]); !swapFormIDs.empty()) {
				...
				// randomly assign each baseFormID to a unique swapFormID
					...
					_ERROR("\t\t\t\tfail : [%s] (SWAP formIDSet.size() must be 1 OR equal/greater than BASE formIDSet.size())", a_str.c_str());
				}
			} else {
				_ERROR("\t\t\t\tfail : [%s] (SWAP formID set not found)", a_str.c_str());
			}
		} else {
			_ERROR("\t\t\t\tfail : [%s] (BASE formID not found)", a_str.c_str());
		}
	}
}
